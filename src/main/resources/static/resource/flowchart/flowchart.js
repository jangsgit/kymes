...
function createFlowChart() {
    var result = $.widget("flowchart.flowchart", {
        // default options
        options: {
        ...
        _refreshLinkPositions: function (linkId) {
            ...
            */
            ...
            fromY = fromY - 12;
            toY = toY - 12;
            bezierFromX = (fromX + offsetFromX + distanceFromArrow - 2);
            bezierToX = (toX + offsetFromX + distanceFromArrow - 2);
        ...
        _getOperatorFullElement: function (operatorData) {
            ...
            if (infos.nodetype == 'node_comment_map') {
                $operator.addClass('node-map-div');
                $operator.resizable({
                    ...
                    stop: function (event, ui) {
                        operatorData.width = ui.size.width;
                        operatorData.height = ui.size.height;
                    }
                });
            } else if (infos.nodetype.includes('node_comment_point')) {
                $operator.addClass('node-point-div');
            }
            ...
            if (infos.nodetype == 'node_comment_map') {
                ...
                $operator_title.addClass('node-map-div-title');
            } else if (infos.nodetype.includes('node_comment_point')) {
                $operator_title.html('');
                $operator_title.addClass('node-point-div-title');
            } else {
                $operator_title.html(infos.title);
            }
        ...
        _createSubConnector: function (connectorKey, connectorInfos, fullElement) {
            ...
            if (connectorInfos.isLast != false) {
        ...
        createOperator: function (operatorId, operatorData) {
            ...
            if (operatorData.properties.nodetype == 'node_comment_map') {
                if (operatorData.properties.mapimage != '') {
                    fullElement.operator.css({ top: operatorData.top, left: operatorData.left, width: operatorData.width, height: operatorData.height, 'background-image': 'url(/static/img/flowchart/' + operatorData.properties.mapimage + ')' });
                } else {
                    fullElement.operator.css({ top: operatorData.top, left: operatorData.left, width: operatorData.width, height: operatorData.height });
                }
            } else {
            ...
            function operatorChangedSize(operator_id, size) {
                operatorData.width = size.width;
                operatorData.height = size.height;
            }
            ...
            if (this.options.canUserMoveOperators) {
                ...
                fullElement.operator.draggable({
                    ...
                    start: function (e, ui) {
                        ...
                        pointerWidth = parseInt($(e.target).css('width'));
                        pointerHeight = parseInt($(e.target).css('height'));
                    },
                    drag: function (e, ui) {
                        if (self.options.grid) {
                            ...
                            fullElement.operator.css({ left: ui.position.left, top: ui.position.top, width: pointerWidth, height: pointerHeight });
                        }
                        operatorChangedPosition($(this).data('operator_id'), ui.position);
                        operatorChangedSize($(this).data('operator_id'), { width: pointerWidth, height: pointerHeight });
                    },
                    stop: function (e, ui) {
